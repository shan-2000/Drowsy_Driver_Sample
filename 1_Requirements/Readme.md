...
## Objective
 ...
 ## Advantages
  ...
  • Record driving data, collision data and position data. 
  Analyze the accidents detail. 
  
  • Send location of car and its maintenance to base station through GPS & GSM technique. 
  
  • Sense gas & fuel leakage and display its status on car monitoring system. 
  
  • Detect if the driver is drunk or not. 
...
__HOW:__
 ...
 The driver drowsiness detection is based on an algorithm, which begins recording the driver's steering behavior the moment when the driver starts to drive. It then recognizes changes over the course of long journey
